...
public class ProductRestController {
    ...
    public ResponseEntity<?> findAll() {
        List<ProductListItemDTO> responseDTOList = new ArrayList<>();

        responseDTOList.add(new ProductListItemDTO(
    ...
    public ResponseEntity<?> findById(@PathVariable int id) {
        ...
        if (id == 1) {
            ...
            productDTO = new ProductDTO(1, "기본에 슬라이딩 지퍼백 크리스마스/플라워에디션 에디션 외 주방용품 특가전", "", "/images/1.jpg", 1000, 5, optionDTOList);
        } else if (id == 2) {
            List<ProductOptionDTO> optionDTOList = new ArrayList<>();
            ...
            productDTO = new ProductDTO(2, "[황금약단밤 골드]2022년산 햇밤 칼집 밤700g외/군밤용/생율", "", "/images/2.jpg", 2000, 5, optionDTOList);
        } else {
